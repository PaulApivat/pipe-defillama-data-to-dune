...
class IncrementalPipeline:
    ...
    def __init__(
        ...
        self.target_projects = target_projects or [
            "pancakeswap-amm",
            "pancakeswap-amm-v3",
        ]
    ...
    def run_initial_load(self) -> bool:
        ...
        try:
            ...
            self.incremental_fetcher.save_run_state(
                datetime.now(), raw_pools_df.height, raw_tvl_df.height
            )
    ...
    def run_incremental_update(self, target_date: Optional[date] = None) -> bool:
        ...
        try:
            ...
            if self.incremental_fetcher.should_fetch_tvl(target_date):
                logger.info(
                    f"📊 Step 2: Fetching incremental TVL data for {target_date}..."
                )
                pool_ids = raw_pools_df.select("pool").to_series().to_list()
                raw_tvl_df = self.incremental_fetcher.get_incremental_tvl_data(
                    pool_ids, target_date
                )
                logger.info(
                    f"✅ Fetched {raw_tvl_df.height} TVL records for {target_date}"
                )
            else:
                logger.info(
                    f"📊 Step 2: TVL data for {target_date} is fresh, skipping fetch"
                )
            ...
            logger.info(
                f"✅ Created {historical_facts_df.height} facts for {target_date}"
            )

            # Step 4: Upload to Dune (incremental)
            if not self.dry_run and historical_facts_df.height > 0:
                logger.info(
                    f"☁️ Step 4: Upserting incremental data for {target_date}..."
                )
...
def main():
    ...
    parser.add_argument(
        "--date", type=str, help="Target date for incremental mode (YYYY-MM-DD)"
    )
