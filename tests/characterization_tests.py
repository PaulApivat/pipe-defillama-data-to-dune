...
class TestStep1TVLDataFetch:
    ...
    def test_tvl_data_fetch_produces_expected_schema(self):
        ...
        assert tvl_data.df.schema == HISTORICAL_TVL_SCHEMA, f"Schema mismatch: {tvl_data.df.schema} != {HISTORICAL_TVL_SCHEMA}"
    ...
    def test_tvl_data_has_expected_volume(self):
        ...
        assert 400_000 <= record_count <= 600_000, f"Unexpected record count: {record_count}"
        ...
        print(f"✅ TVL volume validation passed: {record_count} records, {min_date} to {max_date}")
        return tvl_data
...
class TestStep2SCD2DimensionsFetch:
    ...
    def test_scd2_dimensions_fetch_produces_expected_schema(self):
        ...
        assert scd2_df.schema == POOL_DIM_SCD2_SCHEMA, f"Schema mismatch: {scd2_df.schema} != {POOL_DIM_SCD2_SCHEMA}"
        
        # Verify expected columns
        expected_columns = ["pool_id", "protocol_slug", "chain", "symbol", "valid_from", "valid_to", "is_current", "attrib_hash", "is_active"]
        for col in expected_columns:
    ...
    def test_scd2_dimensions_have_expected_volume(self):
        ...
        assert 1_000 <= record_count <= 2_000, f"Unexpected record count: {record_count}"
        
        # Verify all records are current
        is_current_count = scd2_df.select(pl.col("is_current").sum()).item()
        assert is_current_count == record_count, f"Not all records are current: {is_current_count}/{record_count}"
        
        # Verify valid_from is set to historical date
        valid_from_values = scd2_df.select(pl.col("valid_from").unique()).to_series().to_list()
        assert all(v == date(2022, 1, 1) for v in valid_from_values), f"Invalid valid_from dates: {valid_from_values}"
...
class TestStep3DataJoin:
    ...
    def test_historical_facts_creation_produces_expected_schema(self):
        ...
        # Verify schema matches
        assert historical_facts.schema == HISTORICAL_FACTS_SCHEMA, f"Schema mismatch: {historical_facts.schema} != {HISTORICAL_FACTS_SCHEMA}"
        
        # Verify expected columns
        expected_columns = ["timestamp", "pool_old_clean", "pool_id", "protocol_slug", "chain", "symbol", "tvl_usd", "apy", "apy_base", "apy_reward", "valid_from", "valid_to", "is_current", "attrib_hash", "is_active"]
        for col in expected_columns:
            assert col in historical_facts.columns, f"Missing column: {col}"
        
        print(f"✅ Historical facts schema validation passed: {historical_facts.height} records")
    ...
    def test_historical_facts_has_expected_volume(self):
        ...
        print(f"✅ Historical facts volume validation passed: {record_count} records, {min_date} to {max_date}")
        return historical_facts
...
class TestStep4DuneUpload:
    ...
    def test_dune_uploader_uploads_data(self):
        ...
        try:
            ...
            historical_facts = pl.read_parquet("output/historical_facts_2025-09-23.parquet")
...
class TestEndToEndPipeline:
    ...
    def test_complete_pipeline_produces_expected_output(self):
        ...
        historical_facts = tvl_instance.create_historical_facts(scd2_df)
        assert historical_facts.height > 400_000, f"Historical facts too small: {historical_facts.height}"
...
def run_all_characterization_tests():
    ...
    try:
        ...
        results["scd2_schema"] = test_scd2.test_scd2_dimensions_fetch_produces_expected_schema()
    ...
    try:
        ...
        results["join_schema"] = test_join.test_historical_facts_creation_produces_expected_schema()
